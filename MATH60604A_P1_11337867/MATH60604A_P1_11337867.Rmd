...
output:
  pdf_document:
    ...
    fig_caption: true
fontsize: 5pt
geometry: "left=0.8cm,right=0.8cm,top=0.8cm,bottom=2cm"
header-includes:
   - \usepackage{booktabs}
   - \usepackage{graphicx}
   - \usepackage{amsmath}
...
\begin{tabular}{cccc}
  ...
  \textit{(11343945)} & \textit{(11345395)} & \textit{(11345446)} & \textit{(11337867)} \\
\end{tabular}
\end{center}
\vspace{-2em}
...
```{r bixi rides analysis plots, fig.width=8, fig.height=4.5, echo=TRUE}
...
```{r mem dist plot eda, fig.height=3, fig.width=8, echo=TRUE}
dur_mem_box_plot
```
...
```{r plot above printed, fig.height=3.5, fig.width=9, echo=TRUE}
...
```{r plotting_residual_vs_fitted, fig.width=8, fig.height=4, echo=TRUE}
...
```{r Q3 comparison grid, fig.width=10, fig.height=4, echo=TRUE}
